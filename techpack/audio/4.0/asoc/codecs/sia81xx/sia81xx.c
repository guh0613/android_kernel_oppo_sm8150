	...
	struct sia81xx_dev_s *sia81xx)
{
	...
	if(0 == sia81xx->disable_pin) {
		...
		udelay(10);
		if (0 == sia81xx->channel_num) {
			buf = 0xE0;
			if (0 != sia81xx_regmap_write(sia81xx->regmap, 0x02, 1, &buf))
				pr_err("[  err][%s] %s: write classd reg:0xE0 err !!! \r\n", 
					LOG_FLAG, __func__);
		} else if (1 == sia81xx->channel_num) {
			buf = 0xE8;
			if (0 != sia81xx_regmap_write(sia81xx->regmap, 0x02, 1, &buf))
				pr_err("[  err][%s] %s: write classd reg:0xE8 err !!! \r\n", 
					LOG_FLAG, __func__);
		}

		mdelay(40); /* wait  */
		buf = 0x00;
		if (0 != sia81xx_regmap_write(sia81xx->regmap, 0x06, 1, &buf)){
			pr_err("[  err][%s] %s: write test reg:0x00 err !!! \r\n", 
				LOG_FLAG, __func__);
		}
	...
	char* buf)
{
	...
	if (0 != sia81xx_regmap_read(sia81xx->regmap, 0x41, 1, &chip_id)) {
		pr_debug("[debug][%s] %s: err regmap_read \r\n", LOG_FLAG, __func__);
	}

	if (0 != sia81xx_regmap_read(sia81xx->regmap, 0x00, 0x0D + 1, vals)) {
		pr_debug("[debug][%s] %s: err regmap_read \r\n", LOG_FLAG, __func__);
	}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("[debug][%s] %s: ucontrol = %ld, rst = %d, channel_num = %d \r\n", 
		LOG_FLAG, __func__, ucontrol->value.integer.value[0], sia81xx->rst_pin, sia81xx->channel_num);
	...
	struct snd_soc_codec *codec)
{
	...
	snd_soc_dapm_ignore_suspend(dapm, "SpkrMonoL IN");
	snd_soc_dapm_ignore_suspend(dapm, "SpkrMonoR IN");
	snd_soc_dapm_ignore_suspend(dapm, "SpkrMonoL SPKR");
	snd_soc_dapm_ignore_suspend(dapm, "SpkrMonoR SPKR");
