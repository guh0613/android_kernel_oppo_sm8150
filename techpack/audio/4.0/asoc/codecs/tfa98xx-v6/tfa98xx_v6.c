...
static char fw_name[FW_NAME_LEN] = {0};
static const char *fw_name_default = "tfa98xx_default.cnt";
static char *fw_name_param = "tfa98xx.cnt";
module_param(fw_name_param, charp, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(fw_name_param, "TFA98xx DSP firmware (container file) name.");
#endif /* OPLUS_ARCH_EXTENDS */
...
static enum Tfa98xx_Error tfa9874_calibrate(struct tfa98xx *tfa98xx_cal, int *speakerImpedance)
{
	...
	if (tfa98xx->tfa->dev_idx == 0) {
		...
		if (tfa98xx_device_count == 2) {
			...
			imp = (tfa_get_calibration_info_v6(tfa98xx->tfa, 1));
			list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
			     ...
			     const struct i2c_device_id *id)
{
	...
	ret = of_property_read_string(i2c->dev.of_node,
			"tfa98xx_fw_name", &fw_name_dt);
	if (ret != 0) {
		//no dt specified, use default
		snprintf(fw_name, sizeof(fw_name), fw_name_default);
	} else {
		snprintf(fw_name, sizeof(fw_name), fw_name_dt);
	}
