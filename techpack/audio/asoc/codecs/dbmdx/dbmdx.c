...
#ifdef OPLUS_ARCH_EXTENDS
	dbmdx_enable_micbias(false);
	msleep(100);
	dbmdx_enable_micbias(true);
	dev_err(p->dev, "%s: disable/enable micbias\n", __func__);
#endif /* OPLUS_ARCH_EXTENDS */
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	if (g_event_id_dbmdx_card_fail) {
		...
		scnprintf(payload, sizeof(payload), "EventID@@%d$$dbmdx_card_fail",
	...
	if (g_event_id_dbmdx_spi_fail) {
		...
		scnprintf(payload, sizeof(payload), "EventID@@%d$$dbmdx_spi_fail",
	...
	if (g_event_id_dbmdx_chip_id_fail) {
		...
		scnprintf(payload, sizeof(payload), "EventID@@%d$$dbmdx_chip_id_fail",
	...
	if (g_event_id_dbmdx_fw_ver_fail) {
		...
		scnprintf(payload, sizeof(payload), "EventID@@%d$$dbmdx_fw_ver_fail",
...
static void dbmdx_uevent_work(struct work_struct *work)
{
	...
	if (ret < 0) {
		#ifdef OPLUS_ARCH_EXTENDS
		#ifdef CONFIG_OPPO_KEVENT_UPLOAD
		scnprintf(payload, sizeof(payload), "EventID@@%d$$dbmdx_detection_irq_fail",
...
#if 0 //def OPLUS_ARCH_EXTENDS TBD
	if (is_project(OPPO_19031) && (cmp_pcb("DVT2") < 0)) {
		dev_info(dev, "%s: disable dmic clk for devices before DVT2\n", __func__);
		pdata->va_mic_config[0] = 0x0000;
	} else {
		dev_info(dev, "%s: is 19031? %d, is DVT2 device? %d\n", __func__, is_project(OPPO_19031), cmp_pcb("DVT2"));
	}
#endif /* OPLUS_ARCH_EXTENDS */
	for (i = 0; i < VA_MIC_CONFIG_SIZE; i++)
		dev_dbg(dev, "%s: VA mic cfg %8.8x: 0x%8.8x\n",
