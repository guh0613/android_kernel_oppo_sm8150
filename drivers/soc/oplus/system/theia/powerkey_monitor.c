...
{
    char buffer[128] = {0};
    if (count > 128 || copy_from_user(buffer, buf, count)) {
        POWER_MONITOR_DEBUG_PRINTK("%s: read proc input error.\n", __func__);
        return count;
    }

    POWER_MONITOR_DEBUG_PRINTK("theia_powerkey_test_node_proc_write buffer:%s\n", buffer);
    if (!strcmp(buffer, "test_d_block\n")) {
        ...
        set_current_state(TASK_UNINTERRUPTIBLE);
         ...
         block_thread = NULL;
    } else if (!strcmp(buffer, "test_d_unblock\n")) {
    ...
    } else if (!strcmp(buffer, "test_d_unblock_with_kill\n")) {
/*
        if (block_thread != NULL) {
...
*/
    } else if (!strcmp(buffer, "test_blackscreen_dcs\n")) {
        send_black_screen_dcs_msg();
    } else if (!strcmp(buffer, "test_kevent\n")) {
