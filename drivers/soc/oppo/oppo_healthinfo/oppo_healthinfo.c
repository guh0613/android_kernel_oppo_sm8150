...
void ohm_para_init(void)
{
        int i;
        for (i = 0 ;i < OHM_SCHED_TOTAL ;i++ ) {
               _ohm_para_init(&oppo_sched_para[i]);
...
static ssize_t sched_latency_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
    ...
    int len = 0;
    struct sched_stat_para *sched_stat = &oppo_sched_para[OHM_SCHED_SCHEDLATENCY];
...
static ssize_t iowait_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
    char page[BUFFER_SIZE_L] = {0};
    int len = 0;
    struct sched_stat_para *sched_stat = &oppo_sched_para[OHM_SCHED_IOWAIT];
...
static ssize_t fsync_wait_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
    char page[BUFFER_SIZE_L] = {0};
    int len = 0;
    struct sched_stat_para *sched_stat = &oppo_sched_para[OHM_SCHED_FSYNC];
...
static ssize_t emmcio_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
      int len = 0;
      char page[1024] = {0};
      struct sched_stat_para *sched_stat = &oppo_sched_para[OHM_SCHED_EMMCIO];
...
static ssize_t dstate_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
        char page[BUFFER_SIZE_L] = {0};
        int len = 0;
        struct sched_stat_para *sched_stat = &oppo_sched_para[OHM_SCHED_DSTATE];
...
static ssize_t sdinfo_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
    char page[1024] = {0};
    int len = 0;
    ...
    len += sprintf(&page[len], "manfid: %d\ncsd_version: %d\ncapacity: %d\n"
                ...
                sdinfo.supported_bus_mode, sdinfo.sd_bus_speed, sdinfo.taac_ns,
                sdinfo.taac_clks, sdinfo.r2w_factor, sdinfo.runtime_resume_error_count,
                sdinfo.cmd_timeout_count, sdinfo.cmd_crc_err_count, sdinfo.data_timeout_int_count,
                sdinfo.data_crc_err_count, sdinfo.data_timeout_count, sdinfo.vold_timeout_count);
...
#ifdef OPLUS_FEATURE_SDCARD_INFO
	pentry = proc_create("sd_info", S_IRUGO, oppo_healthinfo, &proc_sdinfo_fops);
	if(!pentry) {
