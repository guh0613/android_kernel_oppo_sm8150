...
static void handle_vdm_rx(struct usbpd *pd, struct rx_msg *rx_msg)
{
	...
	case SVDM_CMD_TYPE_RESP_ACK:
		...
		case USBPD_SVDM_DISCOVER_IDENTITY:
			...
			if (num_vdos == 3){
				oppo_svid = vdos[0] & 0xffff;
				printk("handle_vdm_resp_ack oppo_svid:%d\n",oppo_svid);
				if(oppo_svid == OPPO_SVID){
					handler = find_svid_handler(pd, oppo_svid);
					if (handler) {
						usbpd_err(&pd->dev, "oppo_svid SVID: 0x%04x vdos1: %x vdos2 %x connect\n",
